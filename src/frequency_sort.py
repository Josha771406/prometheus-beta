...
def sort_by_frequency(numbers: List[int]) -> List[int]:
    ...
    Returns:
        List[int]: Sorted list of integers based on their frequency (ascending)
    ...
    # Sort the list based on frequency (ascending) and then by the original order
    return sorted(numbers, key=lambda x: (frequency[x], numbers.index(x)))